...
**NEGATIVE CONSTRAINT:** DO NOT route purely mathematical expressions, algebra, or short variable equations to `CODE_SIMPLE`. Even if they look like variables (e.g. `x + y`), if it is math, it MUST route to `MATH` or `GENERAL`.
