...
def classify_task(
    ...
    # --- Hardcoded Intercepts for Coding / Web Dev ---
    lower_query = query_for_classification.lower()
    
    # 1. Full-Stack / Skeleton / Complex Code Rules
    code_keywords = [r"\bbackend\b", r"\bserver\b", r"\bapi\b", r"\bfull stack\b", r"\bfull-stack\b", r"\bnode\b", r"\bexpress\b", r"\bskeleton\b", r"\bهيكل\b", r"\bقالب\b", r"\bفراغات\b"]
    ...
    web_intent = [r"\bbuild\b", r"\blanding page\b", r"\bwebsite\b", r"\bموقع\b", r"\bصفحة\b", r"\bصمم\b", r"\bبرمج\b"]
    if any(re.search(tech, lower_query) for tech in web_tech) and any(re.search(intent, lower_query) for intent in web_intent):
